    ...
    if (location.hostname === 'live.bilibili.com') {
        ...
        Live.Queue = function (which) {
            ...
            o.remove = function (appoint) {
                let index = o[appoint.state].indexOf(appoint);
                if (index != -1) {
        ...
        Live.Appoint = function (which, rate, number, state) {
            ...
            o.create_dom = function (top) {
                if (o.dom === undefined) {
                    let rate_dom = $('<span>').addClass('rate').text(rate);
                    let number_dom = $('<h4>').addClass('number').text(Live.numFormat(o.number));
                    ...
                    o.updateMenu();
                    let success_dom = Live.bet[which + '_box'].children('.success:last');
                    if (top) {
                        if (success_dom.length != 0) {
                            success_dom.after(o.dom);
                        } else {
                            Live.bet[which + '_box'].prepend(o.dom);
                        }
                    } else {
                        Live.bet[which + '_box'].append(o.dom);
                    }
            ...
            o.run = function (bet) {
                ...
                if (!o.dom.hasClass('run')) {
                    o.dom.addClass('run');
                }

                /* get data*/
                let w = (o.which === 'blue') ? 'a' : 'b';
                let bankerId = bet.silver[w].id;
                let rate = bet.silver[w].times;
            ...
            o.error = function (data) {
                ...
                o.state = 'error';
                console.error(o.id + ':' + data.msg);
                /* deal style class*/
                o.dom.removeClass('cancel wait run success');
                if (!o.dom.hasClass('error')) {
                    o.dom.addClass('error').attr('title', data.msg);
                }
            ...
            o.success = function () {
                ...
                if (!o.dom.hasClass('success')) {
                    o.dom.addClass('success');
                }
            ...
            o.cancel = function () {
                ...
                if (!o.dom.hasClass('cancel')) {
                    o.dom.addClass('cancel');
                }
        ...
        /*Live.bet = {
            ...
            checkBetStatus: function (callback) {
                ...
                Live.bet.getBet().done(function (bet) {
                    ...
                    if (store.get('bilibili_helper_quiz_autoMode')[Live.roomId]) {
                        if (Live.bet.hasInit && !Live.bet.hasShow) {
                            Live.bet.show();
            ...
            show_quiz_helper: function () {
                ...
                if (Live.bet.quiz_helper_height === undefined) {
                    Live.bet.quiz_helper_height = 'auto';
                }
            ...
            initDOM: function () {
                ...
                if (!Live.bet.hasInit) {
                    ...
                    $('#quiz_helper').find('.bet-buy-btns button').click(function () {
                        let which = $(this).attr('data-target');
                        // rate
                        let rate = parseFloat(Live.bet.quiz_rate.val());
                        if (rate.length > 3) {
                            rate = rate.toFixed(1);
                        }
                        var o;
                        ...
                        // number
                        let number = parseInt(Live.bet.quiz_number.val());
                        ...
                        if (Live.bet.quiz_rate.val() === '') {
                            ...
                            Live.bet.quiz_rate.removeClass('error');
                        }
                        if (Live.bet.quiz_number.val() === '') {
                            Live.bet.quiz_number.addClass('error');
                            return;
                        } else {
                            Live.bet.quiz_number.removeClass('error');
                        }
                    ...
                    Live.bet.quiz_number.keyup(function () {
                        let v = $(this).val();
                        let silver = parseInt(store.get('bilibili_helper_userInfo')['silver']);
                        while (v != '' && isNaN(v)) {
            ...
            do: function () {
                Live.bet.getBet().done(function (bet) {
                    ...
                    // deal with run queue
                    let blue = Live.bet.blue_queue.run[0];
                    let red = Live.bet.red_queue.run[0];
                    if (blue) {
                        blue.dealWith(bet);
                    }
                    if (red) {
            ...
            check: function () {
                let o = store.get('bilibili_helper_quiz_check');
                Live.bet.cancelCheck();
                if (!o[Live.roomId]) {
        ...
        Live.treasure = {
            ...
            awardBtn: {
                ...
                awarding: function () {
                    ...
                    let btn = Live.treasure.treasureTip.find('.acquiring-panel .get-award-btn');
        ...
        Live.chat = {
            ...
            hideStyle: {
                ...
                liveTitleIcon: {
                    title: '成就头衔',
                    css: '#chat-msg-list .chat-msg .check-my-title{display:none;}',
                    value: 'off',
                },
                mediaIcon: {
                    title: '粉丝勋章',
            ...
            initDanmu: function () {
                ...
                let original_emoji_btn = Live.chat.chat_ctrl_panel.find('.chat-ctrl-btns .btns .emoji');
                let helper_emoji_btn = original_emoji_btn.clone().addClass('helper-emoji');
                original_emoji_btn.before(helper_emoji_btn).remove();

                let original_emoji_list = Live.chat.chat_ctrl_panel.find('.ctrl-panels .emoji-panel');
                let helper_emoji_list = original_emoji_list.clone().addClass('helper-emoji-list');
                original_emoji_list.before(helper_emoji_list).remove();

                let original_hot_words_btn = Live.chat.chat_ctrl_panel.find('.chat-ctrl-btns .btns .hot-words');
                let helper_hot_words_btn = original_hot_words_btn.clone().addClass('helper-hot-words');
                original_hot_words_btn.before(helper_hot_words_btn).remove();

                let original_hot_words_list = Live.chat.chat_ctrl_panel.find('.ctrl-panels .hot-words-panel');
                let helper_hot_words_list = original_hot_words_list.clone().addClass('helper-hot-words-list');
                original_hot_words_list.before(helper_hot_words_list).remove();

                let original_text_area = Live.chat.chat_ctrl_panel.find('.danmu-sender #danmu-textbox');
                ...
                let original_send_btn = Live.chat.chat_ctrl_panel.find('.danmu-sender #danmu-send-btn');
                let helper_send_btn = original_send_btn.clone().addClass('helper-send-btn');
                ...
                Live.chat.helper_text_area.on('keydown', function (e) {
                    let text = Live.chat.helper_text_area.val().trim();
            ...
            initChatHelper: function () {
                ...
                Live.chat.chatDisplayBlock.find('.display-option .option .button').off('click').on('click', function () {
                    let classes = $(this).attr('class').split(' ')[1];
                    if ($(this).hasClass('on')) {
                        return false;
                    }
                        ...
                        setTimeout(function () {
                            chrome.runtime.sendMessage({
                                ...
                                if (Live.mobileVerified) {
                                    ...
                                    Live.helperInfo.setWatcherStatus('error', '初始化失败，无法通过手机绑定检测');
                                }
                            });
                        }, 2000);
                    }
                    ...
                    if (res['value'] === 'on') {
                        if (Live.BBQ && Live.BBQ.level >= 10) {
                            if (typeof Live.user.user_level_rank === 'string') {
                                console.error('UL rank >25000');
                            } else if (Live.user && Live.user.user_level_rank <= 25000) {
                                Live.scriptOptions['beat'] = true;
                            }
                        } else {
                            console.error('NONE BBQ Medal or BBQ level <10');
                        }
                    }
                });
        ...
        Live.giftpackage = {
                ...
                chrome.runtime.sendMessage({
                    ...
                    if (res['value'] === 'on') {
                        ...
                        Live.giftpackage.sendGiftInput.off('keydown').on('keydown', function (e) {
                            if (e.keyCode === 13 && Live.giftpackage.sendGiftInput.val() != '') {
            ...
            initGiftsDOM: function (giftsData) {
                ...
                Live.each(giftsData, function (id) {
                    let gifts = giftsData[id];
                    let giftsDOM = $('<div />').addClass('gift-item-group').attr('gift_id', id);
                    Live.each(gifts, function (index) {
                        let gift = gifts[index];
                        let giftDOM = $('<span />').addClass('package-item').attr({
                            'title': gift.gift_name,
                    ...
                    let lineSendDOM = $('<div />').addClass('send_line').text('清空本行');
                    let container = $('<div />').addClass('container').append(lineSendDOM, giftsDOM);
                    Live.giftpackage.packageContent.append(container);
                    if (gifts.length > 10) {
                        Live.giftpackage.packagePanel.addClass('big');
                    }
                });
                ...
                Live.giftpackage.packageContent.find('.package-item').off('click').on('click', function (e) {
                    ...
                    let gift = Live.giftpackage.giftPackageData[bagId];
                ...
                Live.giftpackage.packageContent.find('.send_line').off('click').on('click', function (e) {
                    let gift_id = $(this).parent().find('.gift-item-group').attr('gift_id');
            ...
            sortGifts: function (giftpackageData) {
                ...
                Live.each(gs, function (i) {
                    ...
                    Live.each(r[gift.gift_id], function (index) {
                        if (gift.expireat < r[gift.gift_id][index].expireat && position >= index) {
                            position = index;
        ...
        Live.helperInfo = {
            setStatus: function (dom, className, statusClass) {
                if (dom.hasClass(statusClass)) {
                    return;
                } else {
                    dom.attr('class', className).addClass(statusClass);
                }
            },
            ...
            initPanel: function () {
                ...
                let helperAnchorInfoPanel = Live.createPanel(roomTitleRow, helperAnchorInfoBtn, 'click', 'helper-anchor-info-panel', 'helperAnchorInfoPanel');
                let area = roomInfo.find('.room-info.v-top:eq(0)');
                let tags = roomInfo.find('.room-info.v-top:eq(1)');
                ...
                chrome.runtime.sendMessage({
                    ...
                    if (res['value'] === 'on') {
                        Live.watcherBtn = $('<div />').addClass('watcher-info').html('<i class="watcher-info-icon" style="background-image:url(' + chrome.extension.getURL('imgs/icon.png') + ')"></i><span class="watcher-info-text">抽奖提示功能</span>');
                ...
                chrome.runtime.sendMessage({
                    ...
                    if (res['value'] === 'on') {
                        Live.treasureBtn = $('<div />').addClass('treasure-info').html('<i class="treasure-info-icon" style="background-image:url(' + chrome.extension.getURL('imgs/icon.png') + ')"></i><span class="treasure-info-text">辅助领瓜子</span>');
